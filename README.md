...
- ToolRouter 可以根据 intent、工具类型、名称、标签、描述和 `input_schema` 选择候选工具
...
- 工具路由仍以规则打分为主，还没有 embedding 召回和 LLM rerank
- 数据库结构仍由 `SCHEMA_SQL` 初始化，还没有 Alembic migration
- 审批流已经可用，但还没有登录用户、RBAC、审批过期和审批范围控制
- Demo 已覆盖 MCP / HTTP / CLI / Sandbox，但还需要整理成更完整的演示脚本和面试讲解材料
...
- ToolRouter + PermissionEngine：根据意图、工具类型、schema、风险等级和 run_mode 选择并检查工具
